...
def transform_grasp_pose(
    ...
    """
    # Convert grasp pose to a 4x4 matrix
    grasp_pose = to_pose(grasp.trans, grasp.rot)

    # Convert estimated pose to a 4x4 matrix
    est_pose = to_pose(est_trans, est_rot)

    # Convert camera pose to a 4x4 matrix
    cam_pose = to_pose(cam_trans, cam_rot)

    # Transform grasp pose to the camera frame
    grasp_in_camera_frame = est_pose @ grasp_pose

    # Transform grasp pose to the robot frame
    grasp_in_robot_frame = cam_pose @ grasp_in_camera_frame

    # Extract translation and rotation from the transformed pose
    transformed_trans = grasp_in_robot_frame[:3, 3]
    transformed_rot = grasp_in_robot_frame[:3, :3]

    # Return the transformed grasp
    return Grasp(trans=transformed_trans, rot=transformed_rot, width=grasp.width)
